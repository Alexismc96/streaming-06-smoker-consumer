...
def foodB_callback(ch, method, properties, body):
    ...
    #assign to a float
    foodBtemp[0] = float(foodB_temp[-1]) 
    #add temp to deque
    foodB_deque.append(foodBtemp[0])
       
    #create smoker alert
    if len(foodB_deque) == 20:
        B_temp = (foodB_deque[-1]-foodB_deque[0])
        if B_temp > 1:
            print("!STALL ALERT! FoodB has stalled.")
        else:
            print("FoodB temperature is within range. Current temperature is:", foodBtemp[0])
    else:
        print("Current FoodB temperature is within range at:", foodBtemp[0])
